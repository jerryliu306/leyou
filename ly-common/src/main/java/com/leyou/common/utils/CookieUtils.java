...
public final class CookieUtils {
     ...
     * 域名以“.”分割后的长度
     */
    private static final Integer DOMAIN_NAME_LENGTH = 3;
            ...
            for (Cookie aCookieList : cookieList) {
                if (aCookieList.getName().equals(cookieName)) {
                    if (charset != null && charset.length() > 0) {
                        retValue = URLDecoder.decode(aCookieList.getValue(), charset);
                    } else {
    ...
    public static class CookieBuilder {
        ...
        private String getDomainName(HttpServletRequest request) {
            ...
            if (/* serverName == null || */ "".equals(serverName)) {
                ...
                if (len > DOMAIN_NAME_LENGTH) {
                    // www.xxx.com.cn
                    domainName = domains[len - 3] + "." + domains[len - 2] + "." + domains[len - 1];
                } else if (/* len <= DOMAIN_NAME_LENGTH && */ len > 1) {
                    // xxx.com or xxx.cn
            ...
            //IP与端口的连接符
            String ipJointMarkPort = ":";
            if (/* domainName != null && */ domainName.indexOf(ipJointMarkPort) > 0) {
